<template lang="pug">
  div(id='dwlj')
    dl(v-for='(topicOption, i) in data[0].topicOption' :key='i' :class='{vertical: classVertical[i]}' :ref='"option" + i')
      dt
        span {{ 0 * 3 + (i + 1) + 9 }}.
...
export default {
  ...
  methods: {
    onSelectAnswer: function (event, i, option) {
      ...
      this.$eventbus.$emit('setAnswer', this.myAnswer.join(','), 0)
    },
    updateData: function () {
      this.myAnswer = ['', '', '']
    ...
    autoOptionHeight: function () {
      let topicOption = this.data[0].topicOption
