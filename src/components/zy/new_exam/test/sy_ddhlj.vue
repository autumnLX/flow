<template lang="pug">
  div(id='ddhlj')
    dl(v-for='(item, i) in data' :key='i' :class='{disabled: i > index, active: i === index, vertical: classVertical[i]}')
      dt
        span {{ i + 1 }}.
        div(v-html='item.topicOption.topicStem.replace(/<img([^(src])*src="/g, "$&" + resUrl)')
      dd(v-for='(option, j) in item.topicOption.options' :key='i + "@" + j' @click='onSelectAnswer($event, i, option.option)' :ref='"option" + i')
...
export default {
  ...
  methods: {
    onSelectAnswer: function (event, i, option) {
      ...
      this.$eventbus.$emit('setAnswer', option, i)
    },
    updateData: function () {
      this.$eventbus.$emit('setAnswer', this.myAnswer[this.index], this.index)
    },
    autoScroll: function () {
    ...
    data: function (val, oldVal) {
      if (val.phase !== oldVal.phase) {
        ...
        this.myAnswer.forEach((ma, i) => {
          this.$eventbus.$emit('setAnswer', ma, i)
        })
        this.$nextTick(() => {
  ...
  mounted () {
    this.$eventbus.$on('sy_first', index => {
      this.index = index
    })
    this.myAnswer.forEach((ma, i) => {
      this.$eventbus.$emit('setAnswer', ma, i)
