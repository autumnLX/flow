...
<template lang="pug">
  div(class='container')
    ...
    div(class='flow' v-if='phase < 9 || phase === 11')
      ...
      img(src='../../../guides/assets/Beijing/pic_headset.png')
    ...
    //- 开始考试
    div(class='welcome' v-if='phase === 10') 同学们，欢迎参加沈阳市高级中等学校招生考试英语听说考试。试题共包含五道大题。下面请看第一大题。
    //- popup
...
export default {
  ...
  methods: {
    finishPlay () {
      ...
      this.$eventbus.$emit('tip', '')
      this.$eventbus.$emit('voice', 0)
      this.$eventbus.$emit('playStop')
    },
    handlePlay () {
          ...
          success: () => {
            this.player.play({
              onStart: () => {
                ...
                this.$eventbus.$emit('tip', '听指令')
                this.voiceInterval = setInterval(() => {
                  this.$eventbus.$emit('voice', 40 + Math.random() * 30)
                }, 50)
              },
              onStop: () => {
                this.finishPlay()
    ...
    handleRecord () {
      if (this.phase === 2) {
        ...
        this.$eventbus.$emit('showAdjust', false)
        this.phase = 1

        this.player.load({
          url: '/web/static/examination/BJ_THHD_start.mp3', // 开始录音
          success: () => {
            this.$eventbus.$emit('tip', '听指令')
            this.player.play({
              onStart: () => {
              ...
              onStop: () => {
                ...
                this.recorder.record({
                  ...
                  onStart: () => {
                    this.$eventbus.$emit('tip', '录音中')
                    this.$eventbus.$emit('highlightRec', true)
                    this.$eventbus.$emit('recordStart', duration)
                  ...
                  onStop: () => {
                    ...
                    this.$eventbus.$emit('recordStop')
                    this.$eventbus.$emit('highlightRec', false)
                      ...
                      success: (data) => {
                        ...
                        this.recorder.startReplay({
                          ...
                          onStop: () => {
                            ...
                            this.$eventbus.$emit('tip', '')
                            this.$eventbus.$emit('showAdjust', true)
                            this.phase = 6
                          }
                        })
                      }
  ...
  watch: {
    loaded: function (newVal, oldVal) {
      if (newVal && !oldVal) {
        ...
        setTimeout(() => {
          this.phase = 10
          this.$eventbus.$emit('tip', '请等待')
          this.$eventbus.$emit('showRec', true)
          this.$eventbus.$emit('showPlay', true)
          this.$eventbus.$emit('showVoice', true)

          this.player.load({
            url: '/web/static/examination/new_shengyang/welcome.mp3',
            success: () => {
              this.player.play({
                onStart: () => {
                  this.$eventbus.$emit('playStart')
  ...
  mounted () {
    this.$eventbus.$emit('tip', '请等待')
    this.$eventbus.$emit('countdown', 5000, 'num')
        ...
        this.player.load({
          ...
          success: () => {
            this.player.play({
              ...
              onStop: () => {
                this.finishPlay()
                this.$eventbus.$emit('showRec', false)
                this.$eventbus.$emit('showPlay', false)
                this.$eventbus.$emit('showVoice', false)
                this.$eventbus.$emit('showAdjust', true)
                this.phase = 2
              }
            })
...
@image-base-url: "../../../guides/assets/Beijing";
