...
<template lang="pug">
  div(class='container')
    //- 输入准考证号
    div.phase12(v-if='phase === 12')
      img(src='./assets/images/avatar.png')
      p.h1 考试系统登录
      p.h2 请输入准考证号
      input(placeholder='模拟考试，准考证号可为任意1~8位数字' maxlength='8' v-model='passNumber')
      button(@click='handleGo13') 确定
    //- 核对个人信息
    div.phase13(v-if='phase === 13')
      div.title 请核对个人信息
      div.content
        img(src='./assets/images/avatar.png')
        div
          ...
          div
            div.left
              ...
              span ：{{stuSchool}}
      div.footer
        button(@click='handleGo1') 确定
        button.grey(@click='handleBack12') 错误
    //- 主引导流程
    ...
    //- 开始考试
    div(class='welcome' v-if='phase === 10') 这是银川市2018年初中毕业暨高中阶段招生考试英语考试听力部分，该部分共五题。
    //- popup
...
export default {
  ...
  methods: {
    ...
    handleGo13 () {
      if (this.passNumber) {
        if (/[^0-9]/.test(this.passNumber)) {
          alert('准考证号只能输入数字')
        } else if (this.passNumber.length > 8) {
          alert('准考证号不能超过8位')
        } else {
          this.onSetKeyValue('passNumber', this.passNumber)
          this.phase = 13
        }
      } else {
        alert('请输入准考证号')
    ...
    loaded: function (newVal, oldVal) {
      if (newVal && !oldVal) {
        ...
        setTimeout(() => {
          ...
          this.player.load({
            url: '/web/static/examination/YC_WELCOME.mp3',
            success: () => {
              this.player.play({
                onStart: () => {
                  this.onSetKeyValue('highlightPlay', true)
