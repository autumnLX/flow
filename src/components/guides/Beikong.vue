...
<template lang="pug">
  div(class='container')
    ...
    div(class='flow' v-if='phase < 9 || phase === 11')
      ...
      div(class='btns')
        button(class='btn' :class='{disabled: phase === 1 || phase === 3}' @click='handleRecord' v-if='phase < 5') {{initStatus !== "success" ? "正在初始化..." : "开始录音"}}
    ...
    //- 开始考试
    div(class='welcome' v-if='phase === 10')
      video(src='/web/static/examination/beikong/0_desc.mp4' autoplay='true' width='auto' height='100%' @ended='handleWelcomeVideoEnd' ref='introVideo')
        p
          a(href='/web/static/examination/beikong/0_desc.mp4' target='_blank') 您的浏览器不支持 video 标签,请下载视频。
    //- popup
...
export default {
  name: 'GuideOfBeijing',
  props: ['aiPanel', 'player', 'recorder', 'initStatus', 'loaded', 'onSetKeyValue', 'onLoad', 'onStart'],
  data() {
    return {
      phase: 1,
      voiceInterval: -1,
                ...
                this.recorder.record({
                  ...
                  onStop: () => {
                    ...
                    this.recorder.getScores({
                      ...
                      success: (data) => {
                        try {
                          let audioUrl = 'https://' + data[this.lastRecordId].audioUrl.replace(/(https?:\/\/|:[0-9]*)/g, '') + '.mp3'
                          this.player.load({
                            url: audioUrl,
                            success: () => {
                              this.player.play({
  ...
  watch: {
    ...
    initStatus: function (oldVal, newVal) {
      if (newVal) {
        clearInterval(initInterval)
        if (this.initStatus === 'success') this.handleStart()
      }
    },
  },
  mounted() {
